...
import { FileText, ChevronRight, Clock, User } from 'lucide-react-taro'
                ...
                <View className="flex-1 min-w-0">
                  ...
                  {report.status === 'rejected' && report.reject_reason && (
                    <View className="mt-2 bg-red-50 rounded-lg p-2">
                      <Text className="block text-xs text-red-600 line-clamp-2">退回原因：{report.reject_reason}</Text>
                    </View>
                  )}
                </View>
                <ChevronRight size={20} color="#d1d5db" className="shrink-0 mt-2" />
              </View>
            </CardContent>
          </Card>
        )
